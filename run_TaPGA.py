...
# %%
# 'fs_data' chứa các bài toán với số lượng cảm biến cố định.
with open("fix_sensors.pkl", 'rb') as f:
  fs_data = pickle.load(f)
# 'ft_data' chứa các bài toán với số lượng mục tiêu cố định.
with open("fix_targets.pkl", 'rb') as f:
  ft_data = pickle.load(f)


# %%
# HÀM KIỂM TRA MỘT MỤC TIÊU CÓ NẰM TRONG VÙNG PHỦ SÓNG
...
# %% [markdown]
# # Model
# Định nghĩa các thành phần cốt lõi của mô hình Giải thuật Di truyền Lai (Hybrid Genetic Algorithm - HGA)
# %%
...
# %%
# LỚP ĐẠI DIỆN CHO MỘT CÁ THỂ TRONG QUẦN THỂ
class Individual:
...
# LỚP HỖ TRỢ CHO UCB1 (Cơ chế lựa chọn cha mẹ)
class UCB1:
...
# %%
# LỚP CHÍNH CỦA MÔ HÌNH GIẢI THUẬT DI TRUYỀN
class GAModel:
  def __init__(self):
...
def coverage_quality(mask, network, T_matrix):
  ...
  # TÁI CẤU TRÚC: Loại bỏ việc tính toán lại T
  # Ma trận T (T_matrix) được truyền vào, có shape (n, m, q)

  U = np.zeros((n, q, m), dtype=float)
  for i in range(n):
    for j in range(m):
        for p in range(q):
          # Sử dụng T_matrix đã được tính toán trước
          # Lưu ý: T_matrix[i,j,p] (shape n,m,q)
          # U[i,p,j] (shape n,q,m)
          if T_matrix[i, j, p]:
...
for i in range(10):
    ...
    for dt in tqdm.tqdm(fs_data[i]['small']):
        ...
        # SỬA ĐỔI: Truyền model_fs.T vào
        cq_score = coverage_quality(result['result'].genome, dt, model_fs.T)
...
for i in range(10):
    ...
    for dt in tqdm.tqdm(ft_data[i]['small']):
        ...
        DI_score = distance_index(np.asarray(dt['K']), result['result'].achieved_coverage)
        var_score = variance(np.asarray(dt['K']), result['result'].achieved_coverage)
        # SỬA ĐỔI: Truyền model_ft.T vào
        cq_score = coverage_quality(result['result'].genome, dt, model_ft.T)
        act_score = activated_sensors(result['result'].genome, 8)
        di.append(DI_score)
        var.append(var_score)
        cq.append(cq_score)
