...
def is_within_FoV(bisector, target, sensor, radius):
    """
    Kiểm tra xem một mục tiêu có được bao phủ bởi một cảm biến ở một hướng cụ thể không.
    """
...
def init_T(network):
    """
    Khởi tạo ma trận T (lookup table) để lưu trữ thông tin phủ sóng.
    """
...
class FitnessCalculator:
    def __init__(self, network_data):
        ...
        # T_matrix sẽ được tính một lần và truyền vào
    ...
    def calculate_fitness(self, particle):
        """
        Tính toán fitness (để GIẢM THIỂU).
        """
...
if __name__ == "__main__":
    ...
    # 1. Tải Dữ liệu
    print("Đang tải dữ liệu từ fix_sensors.pkl...")
    try:
        with open("fix_sensors.pkl", 'rb') as f:
            fs_data = pickle.load(f)
    except FileNotFoundError:
        print("LỖI: Không tìm thấy file 'fix_sensors.pkl'.")
    ...
    # 3. Chạy thực nghiệm cho fs_small
    print("\n--- Bắt đầu thực nghiệm: Fixed Sensors (Small) ---")
    DI_fs_small = []
    VAR_fs_small = []
    CQ_fs_small = []
    ACT_fs_small = []
    ...
    for i in range(NUM_RUNS):
        ...
        for dt in tqdm(dataset_instances, desc=f"Dataset {i+1}/{NUM_RUNS} (Small)"):
            ...
            cq_score = coverage_quality(best_genome, dt, T_matrix) 
    ...
    DPSO_GA_fs_small = [DI_fs_small, VAR_fs_small, CQ_fs_small, ACT_fs_small]
    with open("DPSO_GA_fs_small.pkl", 'wb') as f:
        pickle.dump(DPSO_GA_fs_small, f)
    print(f"Đã lưu kết quả vào 'DPSO_GA_fs_small.pkl'")

    # --- 4. Chạy thực nghiệm cho fs_large ---
    print("\n--- Bắt đầu thực nghiệm: Fixed Sensors (Large) ---")
    DI_fs_large = []
    VAR_fs_large = []
    CQ_fs_large = []
    ACT_fs_large = []
    ...
    for i in range(NUM_RUNS):
        ...
        dataset_instances = fs_data[i]['large']
    ...
    DPSO_GA_fs_large = [DI_fs_large, VAR_fs_large, CQ_fs_large, ACT_fs_large]
    with open("DPSO_GA_fs_large.pkl", 'wb') as f:
        pickle.dump(DPSO_GA_fs_large, f)
    print(f"Đã lưu kết quả vào 'DPSO_GA_fs_large.pkl'")
    
    print(f"\n--- TẤT CẢ THỰC NGHIỆM HOÀN TẤT ---")
